...
class BookingDetailsController extends Controller
{
    ...
    public function index()
    {
        ...
        $booking_details = \DB::table('trans_booking_details')->whereNotIn('status',[0,6])->get();
    ...
    public function completed($id)
    {
        ...
        \DB::table('trans_booking_details')->where('id', $id)->update(['status' => 6,'job_finish_date'=>date('Y-m-d'),'job_finish_user_id'=>\Auth::id()]);
        \DB::table('booking_moment_details')->where('trans_booking_details_id', $id)->update(['status' => 0]);
        session()->flash('success', 'Booking Detail completed successfully.');
        return redirect(route('master.booking.index'));
    }
    ...
    public function documentupdate($id, Request $request) {
        ...
        $validator = Validator::make(
            $request->all(),
            [
                'ms_exporter_id' => 'nullable|numeric',
    ...
    public function cargo_details($id, Request $request) {
        ...
        $final=[
            ...
            'document_netweight'=>$request->document_netweight,
        ];
    ...
    public function completedlist(Request $request) {
        $booking_details = \DB::table('trans_booking_details')->whereIn('status',[6])->get();
        $data['booking_details'] = $booking_details;
        $countries = \Helper::getCountries();
        $data['countries'] = $countries;
        return view('bookingdetails.completed', $data);
    }
}