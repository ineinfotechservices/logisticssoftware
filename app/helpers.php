...
class Helper
{
    ...
    public static function generate_booking_detail_job_number($increment=1) {
        $ms_port_of_destination =  \DB::table('trans_booking_details')->orderBy('id','desc')->first();
        $company = env('COMPANY_CODE');
        $idx = $ms_port_of_destination->id ?? 0;
        $idnew = $idx + $increment;
        $code = date('Y').'/'.$idnew;
        $new_code = $company.'/'.$code;
        $new_code_check = \DB::table('trans_booking_details')->where('job_number',$new_code)->count();
        if($new_code_check > 0) {
            $tmpid = $increment+1;
