            ...
            <div class="span12 responsive" data-tablet="span12 fix-offset">
                ...
                <div class="portlet box blue">
                    <div class="portlet-title">
                        ...
                        <div class="actions">
                            <a href="{{ route('master.users.add') }}" class="btn green"><i class="icon-plus"></i>
                                Add</a>
                                ...
                                @if (isset($users) && is_object($users) && $users->count())
                                    @foreach ($users as $user)
                                        <tr class="odd gradeX">
                                            ...
                                            <td>{{ $user->name }}</td>
                                            <td>{{ $user->email }}</td>
                                            <td>{{ isset($role_name[$user->roles_id]) ? $role_name[$user->roles_id] : '' }}</td>
                                            <td>
											<a href="{{ route('master.users.edit',['id'=>$user->id]) }}" class="btn green mini"><i class="icon-pencil"></i> Edit</a>
                                            @if(\Auth::id() != $user->id)
											    <a href="javascript:void(0)" onclick=deleteRow('{{ route('master.users.delete',['id'=>$user->id]) }}') class="btn red mini"><i class="icon-trash"></i> Delete</a>
                                            @endif
											</td>
                                        </tr>
                                    @endforeach
