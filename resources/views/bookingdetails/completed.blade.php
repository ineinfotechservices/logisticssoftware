            ...
            <div class="span12 responsive" data-tablet="span12 fix-offset">
                ...
                <div class="portlet box blue">
                    <div class="portlet-title">
                        ...
                        <div class="actions">
                                <a href="{{ route('master.booking.index') }}" class="btn green"><i class="icon-ok-circle"></i>
                                    JOB(s) Listing</a>
                        </div>
                    </div>
                                ...
                                @if (isset($booking_details) && is_object($booking_details) && $booking_details->count())
                                    @foreach ($booking_details as $detail)
                                        <tr class="odd gradeX">
                                            ...
                                            <td>
                                                @if($detail->job_finish_date!='')    
                                                {{ date('d-m-Y',strtotime($detail->job_finish_date)) }}
                                                @endif      
                                            </td>
                                            <td>
                                                @php 
                                                    $finish_user_id = isset($detail->job_finish_user_id) ? $detail->job_finish_user_id : 0;
                                                    $user_info_finish = \Helper::getUserbyId($finish_user_id);
                                                    if(isset($user_info_finish->name)) 
                                                        echo $user_info_finish->name;
                                                @endphp 
                                            </td>
                                            <td style="display:none">
                                                ...
                                                @if (\Helper::isAdmin() || \Helper::isSales())
                                                    ...
                                                    @endif
                                                @endif
                                                @if (\Helper::isAdmin() || \Helper::isSales() || Helper::isDocument())
                                                <a href="javascript:void(0)"
                                                            onclick=finishRow('{{ route('master.booking.completed', ['id' => $detail->id]) }}')
                                                            class="btn red mini"><i class="icon-ok-circle"></i> Finish Job</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
...
<script>
    ...
    const finishRow = (xURL) => {
        if (confirm('Are you sure want to completed this ?')) {
            window.location.href = xURL;
        }
    }
</script>
