            ...
            <div class="span12 responsive" data-tablet="span12 fix-offset">
                ...
                <div class="portlet box blue">
                    <div class="portlet-title">
                        ...
                        <div class="actions">
                            <a href="{{ route('master.state.add') }}" class="btn green"><i class="icon-plus"></i>
                                Add</a>
                                ...
                                @if (isset($states) && is_object($states) && $states->count())
                                    @foreach ($states as $consig)
                                        <tr class="odd gradeX">
                                            ...
                                            <td>
                                            @php 
                                            $countries = \Helper::getCountries($consig->countries_id);
                                            echo $cname = isset($countries->title) ? $countries->title : '';
                                            @endphp 
                                             </td>
                                            <td>
											<a href="{{ route('master.state.edit',['id'=>$consig->id]) }}" class="btn green mini"><i class="icon-pencil"></i> Edit</a>
											<a href="javascript:void(0)" onclick=deleteRow('{{ route('master.state.delete',['id'=>$consig->id]) }}') class="btn red mini"><i class="icon-trash"></i> Delete</a>
											</td>
                                        </tr>
                                    @endforeach
                                @else 
